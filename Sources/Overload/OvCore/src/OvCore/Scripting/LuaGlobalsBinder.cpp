...
void OvCore::Scripting::LuaGlobalsBinder::BindGlobals(sol::state & p_luaState)
{
	...
	p_luaState["Math"]["RandomInt"] = [](int p_min, int p_max) { return OvTools::Utils::Random::Generate(p_min, p_max); };
	p_luaState["Math"]["RandomFloat"] = [](float p_min, float p_max) { return OvTools::Utils::Random::Generate(p_min, p_max); };
	p_luaState["Math"]["CheckPercentage"] = [](float p_percentage) { return OvTools::Utils::Random::CheckPercentage(p_percentage); };
	...
	p_luaState["Physics"]["Raycast"] = [](const OvMaths::FVector3& p_origin, const OvMaths::FVector3& p_direction, float p_distance) -> std::optional<PhysicsWrapper::RaycastHit>
	{
